...
const PackageCommentsModeration: React.FC = () => {
  ...
  const { data: commentsData, isLoading, error } = useQuery({
    queryKey: ['admin-package-comments', filters],
    queryFn: async () => {
      ...
      const response = await axios.get(`/api/admin/packages/comments/?${params}`);
  ...
  const { data: packagesData } = useQuery({
    queryKey: ['admin-packages-for-comments'],
    queryFn: async () => {
      const response = await axios.get('/api/admin/packages/?page_size=100');
      return response.data;
    },
  });
  ...
  const approveMutation = useMutation({
    mutationFn: (id: number) => axios.patch(`/api/admin/packages/comments/${id}/`, { is_approved: true }),
    onSuccess: () => {
      enqueueSnackbar('نظر تایید شد', { variant: 'success' });
      queryClient.invalidateQueries({ queryKey: ['admin-package-comments'] });
    },
    onError: (error: any) => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: number) => axios.patch(`/api/admin/packages/comments/${id}/`, { is_approved: false }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => axios.delete(`/api/admin/packages/comments/${id}/`),
    onSuccess: () => {
  ...
  const bulkActionMutation = useMutation({
    mutationFn: async (data: { action: string; comment_ids: number[] }) => {
      const response = await axios.post('/api/admin/packages/comments/bulk-action/', data);
      return response.data;
    },
    onSuccess: (data) => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      setSelectedComments(comments.map((c: PackageComment) => c.id));
    } else {
      setSelectedComments([]);
    }
  };
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 700, mb: 1 }}>
            مدیریت نظرات پکیج‌ها
          </Typography>
          <Typography variant="body1" color="text.secondary">
            تایید، رد و مدیریت نظرات پکیج‌های آموزشی
          </Typography>
        </Box>
      </Box>
        ...
        <Card sx={{ flex: 1 }}>
          <CardContent sx={{ textAlign: 'center' }}>
            <Typography variant="h4" color="warning.main">
              {comments.filter((c: PackageComment) => !c.is_approved).length}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              در انتظار تایید
            </Typography>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            ...
            <FormControl size="small" sx={{ minWidth: 200 }}>
              <InputLabel>پکیج</InputLabel>
              <Select
                value={filters.package || ''}
                onChange={(e) => handleFilterChange('package', e.target.value || undefined)}
                label="پکیج"
              >
                <MenuItem value="">همه پکیج‌ها</MenuItem>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  comments.map((comment: PackageComment) => (
                    <TableRow key={comment.id} hover>
                      ...
                      <TableCell>
                        <Typography variant="body2">{comment.package_title}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography
                          variant="body2"
...
export default PackageCommentsModeration;
